...
fn price_stream_uses_process_time_sampler_instead_of_block_buckets() {
    ...
    assert!(sampler.contains("pubconstPRICE_SAMPLE_INTERVAL:Duration=Duration::from_secs(30);"));
    assert!(sampler.contains("interval.set_missed_tick_behavior(MissedTickBehavior::Skip);"));

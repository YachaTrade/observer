...
//!
//! These assert that dividend events, wrapped as `V2VaultEvent::Dividend(..)`,
//! flow through the Vault receive path (`vault::receive::process_events`)
//! and preserve the two behaviours that the standalone `V2Dividend` receive
//! guaranteed:
...
//!
//! Pre-implementation these fail to compile: `V2VaultEvent::Dividend` does not
...
use observer::event::v2::vault::receive::process_events;
use observer::types::v2::dividend::{
    DividendClaim, DividendMerkleRoot, DividendSetupEntry, LogCoords, V2DividendEvent,
};
use observer::types::v2::vault::V2VaultEvent;
...
fn div(e: V2DividendEvent) -> V2VaultEvent {
    V2VaultEvent::Dividend(e)
}
...
async fn dividend_events_route_through_vault_receive() -> Result<()> {
    ...
    let events = vec![
        div(V2DividendEvent::Setup(DividendSetupEntry {
        ...
        div(V2DividendEvent::MerkleRoot(DividendMerkleRoot {
            merkle_root: Arc::new(ROOT1.to_string()),
            coords: coords("0xtx_root", 100, 0, 1),
        })),
        div(V2DividendEvent::Claim(DividendClaim {

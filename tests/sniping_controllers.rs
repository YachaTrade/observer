...
async fn sniping_penalties_happy() -> Result<()> {
    ...
    sqlx::query(observer::db::postgres::controller::sniping::INSERT_SNIPING_PENALTIES_SQL)
        .bind(&[TOKEN] as &[&str])
        .bind(&[BUYER] as &[&str])
    ...
    let (sniping_fee, penalty_bps): (BigDecimal, BigDecimal) =
        sqlx::query_as("SELECT sniping_fee, penalty_bps FROM sniping_history WHERE token_id = $1")
            .bind(TOKEN)
            .fetch_one(&db.pool)
            .await?;
...
async fn sniping_penalties_duplicate_ignored() -> Result<()> {
    ...
    let count: (i64,) = sqlx::query_as("SELECT count(*) FROM sniping_history WHERE token_id = $1")
        .bind(TOKEN)
        .fetch_one(&db.pool)
        .await?;
    assert_eq!(count.0, 1);

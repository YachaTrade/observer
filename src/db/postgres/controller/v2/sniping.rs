...
pub struct V2SnipingController {
    pub db: Arc<PostgresDatabase>,
}

impl V2SnipingController {
    pub fn new(db: Arc<PostgresDatabase>) -> Self {
        V2SnipingController { db }
    }

    pub async fn batch_insert_sniping_penalties(
        &self,
        data: &[SnipingPenaltyData],
    ) -> Result<()> {
        if data.is_empty() {
            return Ok(());
        }
        ...
        retry_query("sniping_penalties", || async {
            measure_postgres!("v2_batch_insert_sniping_penalties", {
                sqlx::query(INSERT_SNIPING_PENALTIES_SQL)
                    .bind(&token_ids).bind(&buyers).bind(&sniping_fees).bind(&penalty_bps_list)
                    .bind(&tx_hashes).bind(&block_numbers).bind(&created_ats).bind(&log_indices).bind(&tx_indices)

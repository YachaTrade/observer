        ...
        loop {
            ...
            match measure_postgres!("price_usd_batch_insert", {
                ...
                Err(error) => {
                    ...
                    if error.to_string().to_lowercase().contains("deadlock")
                        && attempt < max_attempts
                    {
                        let delay = base_delay.mul_f32(2.0_f32.powi(attempt - 1));
                        warn!(
                            "[PRICE_USD] Deadlock detected in batch_insert_price_usd, retrying in {}ms",
                            delay.as_millis()
                        );
                        sleep(delay).await;

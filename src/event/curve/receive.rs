...
pub async fn receive_events(
    mut receiver: MonitoredReceiver<CurveEventBatch>,
...
fn group_events_by_token(events: Vec<CurveEvent>) -> HashMap<String, Vec<CurveEvent>> {
    let mut events_by_token: HashMap<String, Vec<CurveEvent>> = HashMap::new();
...
async fn process_token_events(
    ...
    for event in events {
        match &event {
            CurveEvent::Create(create) => {
                account_ids.insert((*create.creator).clone());

                // Cache quote_id for this token
                let _ = cache_manager
                    .insert_token_quote_id(&create.token, &create.quote_id)
                    .await;
    ...
    if let Some(last_sync) = sync_events.last() {
        ...
        if let Err(e) = market_controller
            .handle_curve_sync(&sync_data, &ath_price_usd, ath_price)
            .await
        {

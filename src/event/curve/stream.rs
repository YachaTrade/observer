...
use crate::{
    ...
    types::curve::{
        Buy, CreateCurve, CurveEvent, CurveSync, Graduate, MarketType, Sell, SnipingPenalty,
    },
    utils::{metadata::fetch_token_metadata, to_big_decimal},
};

use super::CurveEventChannel;
...
pub async fn stream_events(event_type: EventType) -> Result<()> {
    ...
    let (channel, receiver) = CurveEventChannel::new("curve_events");
    ...
    loop {
        ...
        let filter = Filter::new()
            ...
            .events(vec![
                ...
                IBondingCurve::SnipingPenalty::SIGNATURE,
            ]);
        ...
        let mut events: Vec<CurveEvent> = Vec::new();
...
async fn parse_log(
    ...
    match log.topic0() {
        ...
        Some(&IBondingCurve::Sync::SIGNATURE_HASH) => {
            ...
            Ok(CurveEvent::Sync(sync))
        }

        Some(&IBondingCurve::Graduate::SIGNATURE_HASH) => {
            let IBondingCurve::Graduate { token, pair } = log.log_decode()?.inner.data;
            ...
            };
            {
                ...
                cache_manager.set_token_pool_flag(&pair, true).await?;
            }
            ...
            Ok(CurveEvent::Graduate(graduate))
        }

        Some(&IBondingCurve::SnipingPenalty::SIGNATURE_HASH) => {
            let IBondingCurve::SnipingPenalty {
                token,
                buyer,
                snipingFee,
                penaltyBps,
            ...
            Ok(CurveEvent::SnipingPenalty(penalty))
        }

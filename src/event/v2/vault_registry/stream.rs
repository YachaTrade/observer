...
use crate::{
    ...
    db::postgres::{PostgresDatabase, controller::v2::V2VaultRegistryController},
    event::get_block_timestamp,
    sync::{BlockRange, EventType, stream::STREAM_MANAGER},
    types::v2::vault_registry::{
        RegisteredVaultType, V2VaultRegistryEvent, VaultDeactivate, VaultRegister,
    },
...
pub async fn stream_events(event_type: EventType) -> Result<()> {
    ...
    loop {
        ...
        let filter = Filter::new()
            ...
            .address(address)
            .events(vec![
                V2VaultRegistry::Register::SIGNATURE,
                V2VaultRegistry::Deactivate::SIGNATURE,
            ]);
        ...
        let mut events: Vec<V2VaultRegistryEvent> = Vec::new();
...
async fn parse_log(log: Log, client: &RpcClient) -> Result<V2VaultRegistryEvent> {
    ...
    match log.topic0() {
        Some(&V2VaultRegistry::Register::SIGNATURE_HASH) => {
            let V2VaultRegistry::Register {
                vault,
                name,
                creator,
                vaultType,
            ...
            Ok(V2VaultRegistryEvent::Register(VaultRegister {
        ...
        Some(&V2VaultRegistry::Deactivate::SIGNATURE_HASH) => {
            let V2VaultRegistry::Deactivate { vault, active } = log.log_decode()?.inner.data;

            Ok(V2VaultRegistryEvent::Deactivate(VaultDeactivate {
...
async fn fetch_vault_details(
    ...
    String,
    Option<crate::types::v2::vault_registry::VaultMetadata>,
)> {

...
use crate::{
    ...
    types::token::{PositionHistoryEvent, TokenEvent},
};
...
pub async fn receive_events(
    ...
    while let Some(events) = receiver.recv().await {
        ...
        // PositionHistory goes directly to its batch insert path; the rest
        // flow through the address-grouped pipeline.
        ...
        warn!(
            "📊 {:?} Receiver: Events: {} | Addresses: {} | Positions: {} | Total: {} | Process time: {}ms | To Block: {} | Latest Block: {}",
            event_type,
            event_count,
            address_count,
            position_count,

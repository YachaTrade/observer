...
use crate::{
    event::{
        core::{EventBatch, EventChannel},
        handler::{EventHandler, run_event_handler},
    },
    sync::EventType,
};

pub type PriceUsdEventBatch = EventBatch<PriceUsdRow>;
pub type PriceUsdEventChannel = EventChannel<PriceUsdRow>;
...
/// A whitelist token resolved for DefiLlama pricing.
///
/// `token_id` is the on-chain address for THIS deployment — the price_usd
/// storage / join key (testnet mock address on testnet, mainnet address on
/// mainnet). `query_id` is the DefiLlama coin address = `COALESCE(price_source_id,
/// token_id)`: equals `token_id` on mainnet, the mainnet equivalent on testnet
/// (DefiLlama only knows mainnet addresses).
#[derive(Debug, Clone, PartialEq)]
...
/// Distinct DefiLlama coin refs to request. Multiple whitelist tokens can share
/// one query address (e.g. MON/LVMON/WMON all price via mainnet WMON), so each
/// address is requested once. First-occurrence order preserved.
pub fn distinct_query_coin_refs(tokens: &[WhitelistToken]) -> Vec<String> {
    let mut seen = HashSet::new();
    tokens
        .iter()
        .filter(|token| seen.insert(token.query_id.clone()))
        .map(|token| coin_ref(&token.query_id))
        .collect()
}

/// Fold fetched DefiLlama prices (keyed by query coin_ref) into the carry-forward
/// `last_good_prices` map keyed by STORAGE `token_id`. One query price fans out
/// to every storage token that uses it. Below-threshold confidence or a missing
/// entry carries the previous good price forward (never overwrites with a bad one).
pub fn apply_fresh_prices(
    ...
    for token in tokens {
        let expected_ref = coin_ref(&token.query_id);
        ...
        match point.confidence.as_ref() {
            Some(confidence) if confidence >= min_confidence => {
                last_good_prices.insert(token.token_id.clone(), point.clone());
            }
            Some(confidence) => warn!(
                "[PRICE_USD] DefiLlama confidence below threshold for {} (token {}): {} < {}; carrying forward",
                expected_ref, token.token_id, confidence, min_confidence
            ),
            None => warn!(

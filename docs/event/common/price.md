...
- `PYTH_MAX_REQUESTS`와 `PYTH_RATE_LIMIT_WINDOW_SECS`는 프로세스 로컬 sliding window를 설정한다.
- 최초 호출과 모든 재시도는 실제 HTTP 전송 직전에 같은 limiter를 통과한다.
- 429 응답은 `PYTH_429_COOLDOWN_SECS`와 유효한 초 단위 `Retry-After` 중 더 긴 시간을 기다린다.
- 개발 설정 `1회/60초`에서는 backfill 처리량이 최대 약 25블록/분이며, 13,500블록은 약 9시간이 걸릴 수 있다.
- 같은 외부 IP의 다른 프로세스는 quota를 공유하므로, 인스턴스가 여러 개면 각 프로세스의 값을 더 낮추거나 분산 limiter를 별도로 도입해야 한다.

...
**ABI**: `abi/BurnVault.json`, `abi/LPVault.json`, `abi/CreatorFeeVault.json`, `abi/GiftVault.json`, `abi/DividendVault.json`
...
### DB 저장
- `vault_burns` 테이블: vault_type (BURN/GIFT), token_id, quote_in, token_burned
...
### DB 저장
- `creator_fee_claims` 테이블: event_type=DEPOSIT, creator=NULL
...
### DB 저장
- `creator_fee_claims` 테이블: event_type=CLAIM, creator=주소, new_balance=NULL
...
### DB 저장
- `creator_updates` 테이블: event_type=SETUP, old_creator=NULL, new_creator=creator
...
### DB 저장
- `creator_updates` 테이블: event_type=UPDATE, old_creator, new_creator

**"토큰 X의 현재 크리에이터" 쿼리**:
```sql
SELECT new_creator FROM creator_updates
WHERE token_id = $1
...
### DB 저장
- `gifts` 테이블: event_type=SETUP, platform, platform_id, expires_at
...
### DB 저장
- `gifts` 테이블: event_type=CLAIM, receiver
...
### DB 저장
- `gifts` 테이블: event_type=RECEIVER_SET, receiver=주소
...
### DB 저장
- `gift_expiry_updates` 테이블 (토큰 스코프 없음)

**"현재 만료 기간" 쿼리**:
```sql
SELECT new_duration FROM gift_expiry_updates
ORDER BY block_number DESC, log_index DESC LIMIT 1;
```
...
`burn_vault_stats`, `lp_vault_stats`, `creator_fee_vault_stats`, `gift_vault_stats`는 이벤트 테이블의 `AFTER INSERT` trigger가 갱신하는 사전 집계 테이블이다. 모두 `token_id`를 key로 사용하며 항상 최신 값을 유지한다.

각 테이블은 누적 금액과 최신 상태를 토큰별로 노출하며, GiftVault는 Accumulating/Active/Burned 상태를 파생한다.

...
**스트림**: `Vault` (DividendVault는 싱글톤 vault이므로 Burn/LP/CreatorFee/Gift와 함께 Vault 멀티플렉서 스트림에서 topic0+주소로 디코딩된다. 전용 Dividend EventType은 없고 페이로드는 `VaultEvent::Dividend(DividendEvent)`로 래핑된다.)
**체크포인트**: `vault`
**컨트랙트**: DividendVault (싱글톤 UUPS — 토큰별 인스턴스 없음)
**ABI**: `abi/DividendVault.json`
...
토큰당 1회 불변 — 재설정 시 `AlreadyConfigured` revert. 따라서 `dividend_setups`가
config 조회 테이블을 겸한다.
...
### DB 저장
- `dividend_setups` 테이블, PK `(transaction_hash, tx_index, log_index, entry_index)`
- 트리거: `dividend_vault_stats`에 `(source_token, dividend_token)` 행 시드 (설정만 되고 자금 흐름이 없는 쌍도 zero-row로 노출)
...
### DB 저장
- `dividend_deposits` 테이블: amount(슬라이스 값), `pending`, `entry_index`, quote_id, usd_value
...
### DB 저장
- `dividend_conversions` 테이블: consumed_quote, received, quote_id, usd_value(= consumed_quote 기준)
...
### DB 저장
- `dividend_merkle_roots` 테이블 + 좌표 `(block_number, tx_index, log_index)` DESC 인덱스 (최신 root 조회용)
...
```sql
SELECT m.merkle_root FROM dividend_merkle_roots m
...
### DB 저장
- `dividend_claims` 테이블: holder, amount(> 0), merkle_root, entry_index, usd_value
...
| 테이블 | 소스 이벤트 | 비고 |
|--------|------------|------|
| `dividend_setups` | DividendSetup (분해) | 불변 config 겸용, stats 행 시드 |
| `dividend_deposits` | Deposit (분해) | 슬라이스당 행, `pending` 플래그 + `entry_index` |
| `dividend_conversions` | Converted (분해) | usd_value는 consumed_quote 기준 |
| `dividend_merkle_roots` | SetMerkleRoot | 좌표 DESC 인덱스 |
| `dividend_claims` | Claim (분해, paid 전용) | merkle_root 귀속 |
| `dividend_vault_stats` | (트리거 파생) | PK (source_token, dividend_token) |
